...
- **Ready for work**: the label `ready-for-agent`. A ticket carrying it is fully specified and safe to hand to an agent. `/swarm` reads this name from here when a bare invocation offers to build a map out of the ready tickets: `gh issue list --state open --label ready-for-agent --search "no:assignee"`. It is the same string as the `ready-for-agent` triage role in `triage-labels.md`; that table stays the place to rename it.
