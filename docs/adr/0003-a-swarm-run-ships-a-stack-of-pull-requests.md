...
**Gate ready on a clean review** was rejected, and `0001-review-does-not-gate.md` carries that decision and its reasons.
...
Marking a layer ready has no REST endpoint, so a run's transport has to reach GraphQL and not REST alone. `docs/agents/issue-tracker.md` carries the operation.
