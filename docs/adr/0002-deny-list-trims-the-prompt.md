...
`permissions.deny` cannot be read as a list of things this environment prevents, and the entries do not all earn the same thing. That is why `environment.sh` keeps the two halves apart with a comment on each, and why `tests/cases/settings-json-shape.sh` asserts the absence of `AskUserQuestion` and `SendMessage` as explicitly as it asserts the presence of the rest — the reverted entries are precisely what a later tidy-up of the list would restore without noticing the cost.
