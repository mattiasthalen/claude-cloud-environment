...
assert_status 1
assert_output_contains "uv tool install prefect==${pin}"
assert_output_contains "!!! step failed: uv tool install prefect==${pin}"

# No fallback to latest: nothing was installed, so nothing is on PATH.
printf '%s\n' "${HARNESS_TOOLS}" | grep -q '^prefect ' &&
  harness_fail "expected no prefect on PATH after a failed install, found: ${HARNESS_TOOLS}"
...
# problem.
assert_output_lacks "✗ prefect"
assert_output_lacks "verify prefect"
