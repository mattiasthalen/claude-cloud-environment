...
for what each one carries and how to obtain it.
...
Three of this repo's agent docs ship the same way — `issue-tracker.md`,
...
| MINOR | A new capability, such as an additional tool that can be requested, a shipped skill or agent doc, or a new standing rule in the memory file every session reads. | Existing box text stays valid; re-tag to pick the change up. |
