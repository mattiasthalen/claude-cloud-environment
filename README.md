...
```
# environment.sh v1.4.0
curl -sL https://raw.githubusercontent.com/mattiasthalen/claude-cloud-environment/refs/tags/v1.4.0/environment.sh | bash -s -- gcloud kubectl gke-gcloud-auth-plugin snow
```
...
`kubelogin`, and the add-on `gke-gcloud-auth-plugin` (which requires `gcloud` in
the same list).
